...
RC initBufferPool(BM_BufferPool *const bm, const char *const pageFileName, const int numPages, ReplacementStrategy strategy, void *stratData){
	...
	bzero(fHandle, sizeof(SM_FileHandle));
	openPageFile((char *)pageFileName, fHandle); //open pageFile

	oldestFrame = 0;	//used for FIFO. Initially point to the first frame
	writeIOCount = 0;
	readIOCount = 0;
	...
	bzero(bufferPoolStatistics, (4 * (sizeof (int*))));
	for(i=0;i<4;i++)
	{
		*(bufferPoolStatistics+i)=(int*)malloc(sizeof(int)*numPages);
		bzero(*(bufferPoolStatistics+i), (sizeof(int)*numPages));
	}
...
bool *getDirtyFlags (BM_BufferPool *const bm){
	int i;
	dirtyFlags = (bool *)malloc(bm->numPages *(sizeof(bool *)));
	bzero(dirtyFlags, sizeof(bool *));
