...
-----------
	...
	2) Run application through user interface:
		Command : make testui
...
Following are the choices provided to the user:	
	...
	2. Open page file
		Opens the page file specified by user. If the file does not present on the disk it gives a message "File does not found". If an already open file is opened it returns an error code saying file is already open.

	3. Close page file
		Closes the page file. Throws an error if file is not open.
	...
	12. Write to a block
		Writes the content to the specified block. Accepts block number as input. It will write only to the existing blocks. If there is a need to write to any non existing block then that block needs to be created fist with appendBlock or ensureCapacity
